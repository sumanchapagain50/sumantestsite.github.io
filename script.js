...
let droppedTilesMap = {}; // key: tile text, value: drop box id
let tilePoints = {};
...
function populateCube() {
  cube.innerHTML = '';
  const isHardMode = gameLevelSelect.value === 'Hard';
  ...
  faces.forEach(face => {
    ...
    const dropBox = document.getElementById(`drop-${face}`);
    if (dropBox) {
  ...
  descriptionBox.textContent = "Click a tile to see description here.";
...
function enableTileEvents() {
  ...
  tiles.forEach(tile => {
    ...
    tile.ondblclick = () => {
      ...
      tile.classList.toggle('draggable');
      tile.style.cursor = draggable ? 'default' : 'grab';
    };
    ...
    box.ondrop = e => {
      ...
      droppedTile.onclick = () => {
        ...
        for (const t of cubeTiles) {
          if (!t.textContent && !t.hasAttribute('data-key')) {
            t.textContent = key;
...
submitBtn.onclick = () => {
  ...
  dropBoxes.forEach(box => {
    ...
    const droppedTiles = [...box.children].filter(c => c.classList.contains('dropped-tile'));
    droppedTiles.forEach(child => {
      const key = child.getAttribute('data-key');
      const actionEntry = Object.entries(floodActions).find(([cat, acts]) =>
        acts.some(act => act.text === key)
      );
      const point = actionEntry
        ? actionEntry[1].find(act => act.text === key)?.point || 0
        : 0;
      const isCorrect = actionEntry && faceTitles[box.dataset.face] === actionEntry[0];
      child.textContent = `${key} (${isCorrect ? '+' : gameMode === "Easy" ? '' : '-'}${point})`;
    });
  });
...
// Tooltip for game level selection
